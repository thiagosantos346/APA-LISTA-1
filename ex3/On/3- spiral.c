...
int main()
{
    ...
    while (resul != valor)
    {
        ...
        if (coordX != coordY && controle1 == 0)
        { //enquanto Y for menor ou diferente que X, soma se +1 ao valor de Y.
            if (resul == valor)
                break;
        ...
        if (controle1 == 1)
        {
            ...
            if (coordX <= coordY && controle2 == 0)
            { //ao chegar ao ponto mais alto da dirreita positiva começa se o decremnto de x até que x*(-1)== Y.
                if (coordX == coordY * (-1))
                {
                    if (resul == valor)
                        break;
