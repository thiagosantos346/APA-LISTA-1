...
/*
Para o problema do quadrado mágico há diversas implementações diferentes, até implementações com algoritmos genéticos,
mas para esse trabalho, foi escolhido um metodo mais direto e simples. Para isso tem que levar em consideração que existem
3 tipos diferentes de quadrados mágicos e para cada um, existe uma forma diferente de solucionar:
1 -> Quadrado mágico Impar, em que o valor de n é impar, chamado de 'Odd Even' (3,5,7,9,...)
2 -> Quadrados que são divisiveis perfeitamente por 4, em que valor de n pode ser encontrado através da formula n = (m*4), 
	 chamados de 'Doubly Even' (4,8,12,16,...)
3 -> Quadrado que são divisiveis por 2, são aqueles que não entram na caso acima, mas ainda são pares e o valor de n é encontrado 
	 através da formula n = 2(2*m+1), são chamados de 'Singly even' (6,10,14,18,...)
*/
...
void doublyEvenSolution(int n){
    ...
    // O proximo passo é alterar as posições de valores dentro da quadrado
    // por posições especificas dentro da matriz seguindo a seguinte regra:  
    // (n*n+1)-square[i][j] 
    
    // Se começa pelo topo esquerda do quadrado  
    // A região que terá seus valores trocados é um quadrado menor com o tamanho (n/4)²
    // dentro do quadrado maior e terá seus valores trocados com os valores que estão
    ...
    // do passo anterior, uma região quadrada (n/4)² tem seus valores substituidos
    // por um quadrado (n/4)² que se encontra no canto inferior direito, de forma espelhada e invertida
    for ( i = 0; i < n/4; i++) 
        for ( j = 3 * (n/4); j < n; j++) 
            square[i][j] = (n*n + 1) - square[i][j]; 
      
    // Agora se faz o mesmo processo em relação ao canto inferior esquerdo, seguindo a mesma
	// ideia dos dois primeiros passos, o que muda é que o vai se basear nos antigos valores
	// do canto superior direito de forma espelhada e invertida
    for ( i = 3 * n/4; i < n; i++) 
        for ( j = 0; j < n/4; j++) 
            square[i][j] = (n*n+1) - square[i][j]; 
     
    // Agora se faz o mesmo processo em relação ao canto inferior direito, seguindo a mesma
	// ideia dos dois primeiros passos, o que muda é que o vai se basear nos antigos valores
    ...
    // e invertida, o quadrado interno que será alterado tem tamanho (n/2)²
    for ( i = n/4; i < 3 * n/4; i++) 
        for ( j = n/4; j < 3 * n/4; j++) 
            square[i][j] = (n*n + 1) - square[i][j]; 
...
void singlyEvenSolution(int n) {
	...
	//Esse quadrado tem uma caracteristicas interessante, pelo n ser um par e não divisivel por 4,
	//a metade de N vai sempre ser um valor impar e usaremos isso para resolver o quadrado mágico.
	//O quadrado de tamanho N terá dentro dele 4 quadrados de tamanho n/2, como eles são impares, se utiliza
	//o oddSolution para preencher esses quadrados impares.
	//Após isso cada quadrante do quadrado maior terá 4 sub quadrados preenchidos de 1 até n/2, para criar 
	//o quadrado mágico é necessario somar um valor em cada numero de cada quadrante, mas esse valor varia por quadrante, por conta do fator de multiplicação
	//No primeiro quadrante(topo esquerda) é somado ((n/2)^2 * fator), sendo fator = 0, 
	//no o segundo quadrante(topo direito) é somado ((n/2)^2 * fator), sendo fator = 2,
	//no terceiro quadrante(base esquerda) é somado ((n/2)^2 * fator), sendo fator = 3,
	//e no ultimo quadrante (base direita) é somado ((n/2)^2 * fator), sendo fator = 1,
    ...
    int subSquaresFactors[] = {0, 2, 3, 1}; //Será utilizado para determinar o valor que será somado ao numero dentro de cada quadrante
    ...
    for (i = 0; i < n; i++) {
 	...
 	//Ainda é necessario mais um passo para se chegar ao quadrado mágico que consiste em
 	//pegar números de posições especificas e trocar com  numeros de outras posições
 	...
 	//E para saber se teremos que alterar os quadrantes 2 e 4, é so usar a formula m-1 e o resultado é a quantidade de colunas que devem ser trocadas
 	// entre os quadrantes 2 e 4.
 	//Exemplo: no caso de n = 6, aplicamos em 6 = 2(2*m-1), temos m = 1, logo 1-1 é 0, logo não se mexe em nenhuma coluna, ja caso n = 10, 
	//teremos m = 2, 2-1 = 1, então teriamos que mexer na ultima coluna dos quadrantes 2 e 4 e inverter eles.
